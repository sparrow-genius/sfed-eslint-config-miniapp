module.exports = {
  ...
  rules: {
    ...
    // --fix 强制在计算的属性的方括号中使用一致的空格:禁止在计算属性内使用空格
    // obj[foo]  obj['foo']  let x = {[b]: a}  obj[foo[bar]]
    ...
    // extends、finally、for、from、function、get、if、import、in、instanceof、let、new、of、return、set
    // static、super switch、this、throw、try、typeof、var、void、while、with 和 yield。
    // 该规则不会与其它空格规则发生冲突：它并不应用于别的规则会报告问题的空格。
    ...
    // 禁用指定的标识符:糟糕的名字会导致代码很难理解。
    // 通用的名字，比如 data，并不能推断出关于代码和接收到的值的更多信息。
    // 该规则允许你配置一个标识符名称的黑名单，也就是包含那些你不想在代码中看到的名字。
    // https://eslint.bootcss.com/docs/rules/id-blacklist/
    // 'id-blacklist': ['obj', 'e', 'cb', 'callback'],
    // 强制标识符的最小和最大长度:默认2，
    // 非常短的标识符名称像 e，x，_t 或非常长的名称像 hashGeneratorResultOutputContainerObject 使代码难以阅读和潜在的不可维护性。
    // 为了防止这种情况出现，应该限制标识符的最大和/或最小长度。
